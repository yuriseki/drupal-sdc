...
- Compare proposed implementations with existing Drupal core examples (e.g., Olivero teaser component)
...
### Schema Validation
- Validate component.yml against the official Drupal SDC metadata schema (base-knowledge/metadata.schema.json)
- Ensure all properties, slots, and library definitions conform to schema requirements
- Check for invalid types, missing required fields, or unsupported structures
- Use JSON Schema validation tools to catch schema errors early
...
3. Compare with existing Drupal core SDC examples (e.g., web/core/themes/olivero/components/teaser)
4. Check each validation point systematically
5. Document findings (compliance and violations)
...
- List any schema errors or warnings with specific references
